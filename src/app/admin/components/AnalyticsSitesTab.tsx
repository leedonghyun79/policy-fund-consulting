...
interface AnalyticsSitesTabProps {
  activeReferrerSites: any[];
}

export default function AnalyticsSitesTab({ activeReferrerSites }: AnalyticsSitesTabProps) {
  const [currentPage, setCurrentPage] = useState(1);
  const ITEMS_PER_PAGE = 10;
  const totalPages = Math.ceil(activeReferrerSites.length / ITEMS_PER_PAGE) || 1;
  const paginatedData = activeReferrerSites.slice((currentPage - 1) * ITEMS_PER_PAGE, currentPage * ITEMS_PER_PAGE);

  return (
    <div style={{ background: '#fff', borderRadius: '24px', border: `1px solid ${THEME.border}`, padding: '32px' }}>
      <h3 style={{ fontSize: '20px', fontWeight: 900, marginBottom: '24px' }}>유입 사이트 분석</h3>
      <table style={{ width: '100%', borderCollapse: 'collapse' }}>
        <thead style={{ borderBottom: `1px solid ${THEME.border}`, background: '#f8fafc' }}>
          <tr>
            <th style={{ padding: '20px 24px', textAlign: 'left', fontSize: '13px', fontWeight: 800, color: THEME.textMuted }}>도메인 (URL)</th>
            <th style={{ padding: '20px 24px', textAlign: 'right', fontSize: '13px', fontWeight: 800, color: THEME.textMuted }}>유입수 (비율)</th>
          </tr>
        </thead>
        <tbody>
          {activeReferrerSites.length === 0 ? (
            <tr>
              <td colSpan={2} style={{ padding: '80px 0', textAlign: 'center', color: THEME.textMuted }}>
            ...
            paginatedData.map((r: any, i: number) => (
              <tr key={i} style={{ borderBottom: `1px solid ${THEME.border}` }}>
                <td style={{ padding: '22px 24px', fontWeight: 800, fontSize: '14px', color: THEME.textMain }}>{r.site}</td>
                <td style={{ padding: '22px 24px', textAlign: 'right', fontWeight: 700, fontSize: '14px' }}>{r.views} <span style={{ color: THEME.textMuted, fontSize: '13px' }}>({r.rate})</span></td>
              </tr>
            ))
          )}
        </tbody>
